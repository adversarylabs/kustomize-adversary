...
test("the published runtime executes without node_modules", async () => {
  const artifact = await mkdtemp(join(tmpdir(), "kustomize-artifact-"));
  const repository = await mkdtemp(join(tmpdir(), "kustomize-target-"));
  ...
  await writeFile(join(repository, "kustomization.yaml"), "resources: []\n");
