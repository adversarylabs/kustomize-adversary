#!/usr/bin/env node
import { createRequire as __kustomizeCreateRequire } from 'node:module'; const require = __kustomizeCreateRequire(import.meta.url);
...
function locate(file, expression, anchors) {
  ...
  for (const match of file.source.matchAll(pattern)) {
    ...
    const line = file.status === "modified" && anchors !== void 0 ? eligibleSemanticAnchor(file, match[0], match.index, anchors) : eligibleLine(file, startLine, endLine);
...
function eligibleSemanticAnchor(file, matchedSource, offset, anchors) {
  for (const anchor of anchors) {
    ...
    for (const match of matchedSource.matchAll(new RegExp(anchor.pattern, flags))) {
      ...
      if (eligible !== void 0) return eligible;
    }
  }
  return void 0;
}
